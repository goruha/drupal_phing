...
role :web, $host
    ...
    task :set_repository, :roles => :web do
        ...
        task :cleanup, :except => { :no_release => true } do
            ...
            if count >= local_releases.length
                ...
                try_sudo "chmod -R a+w #{directories}"
    ...
    task :update_code, :roles => :web do
        on_rollback { run "rm -rf #{release_path}; mysql -h#{$db_host} -u#{$db_user} -p#{$db_pass} #{$db_name} < #{$db_backup_path}#{application}.sql" }
		...
		# backup db
		run "mysqldump --add-drop-table -h#{$db_host} -u#{$db_user} -p#{$db_pass} #{$db_name} > #{$db_backup_path}#{application}.sql"

		# run Drupal installation script
		run "#{release_path}/drush-install.sh #{release_path} #{drush_binary}"
